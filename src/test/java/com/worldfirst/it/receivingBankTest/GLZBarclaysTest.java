...
public class GLZBarclaysTest {
	
	/** Return BarclaysBank for GL E-commerce Client that buys EUR with USD.
	 *  Get the dealId from this test and query database as follows;
	 *  Select recBank from bo_deals where id = <dealId>
	 */ 
	
	private static Logger log = LogManager.getLogger(GLZBarclaysTest.class.getName());
	...
	@Test
	public void quoteGLZBarclaysTest() {
		log.info("***Quote Test***");
		String requestBody = PayloadConverter.generatePayloadStringL2R("GLZClientSendsGBPBuyEUR.json");
		quoteResponse = 
		given()
	...
	@Test (enabled = true, dependsOnMethods = {"quoteGLZBarclaysTest"})
	public void bookGLZBarclaysTest() {
		log.info("***Book Test***");
		bookResponse = 
		given()
