...
public class USCCitiBankTest {
	
	/** Return CitiBank for US Corporate Client that buys EUR with USD.
	 *  Get the dealId from this test and query database as follows;
	 *  Select recBank from bo_deals where id = <dealId>
	 */ 
	
	private static Logger log = LogManager.getLogger(USCCitiBankTest.class.getName());
	...
	@Test
	public void quoteUSCCitiBankTest() {
		log.info("***Quote Test***");
	...
	@Test (enabled = true, dependsOnMethods = {"quoteUSCCitiBankTest"})
	public void bookUSCCitiBankTest() {
		log.info("***Book Test***");
		bookResponse = 
		given()
